...
def test_future_target_prices_do_not_change_the_first_five_minute_forecast() -> None:
    ...
    edited = bars.copy()
    target_future = (pd.to_datetime(edited["timestamp"], utc=True) >= pd.Timestamp("2026-01-19T14:31:00Z"))
...
def test_missing_realized_endpoint_marks_a_forecast_incomplete_without_reweighting() -> None:
    ...
    missing = timestamps.eq(pd.Timestamp("2026-01-19T14:35:00Z"))
...
def test_report_discloses_arithmetic_and_compounded_reconciliation() -> None:
    ...
    assert "Arithmetic sum of expected 5-minute returns" in text
    assert "Compounded actual return" in text
